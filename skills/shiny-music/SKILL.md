...
- Identify songs by listening to audio through the device microphone (Apple platforms via ShazamKit)
...
Identifies songs by listening to audio through the device microphone. On Apple platforms, uses ShazamKit. Android does not currently have an implementation.
...
- Automatically requests microphone permission on Apple platforms via `AVAudioApplication.RequestRecordPermissionAsync`
...
| `MusicUrl` | A URL to the track on a music streaming service (e.g. Apple Music on Apple platforms), or `null` if not available. Platform-neutral name — the actual URL depends on the identification backend. |
...
13. **Stop playback before identifying** — call `IMusicPlayer.Stop()` before `IMusicIdentifier.ListenAsync()` so the microphone captures external audio.
14. **`IMusicIdentifier` is Apple-platforms-only** — only registered via DI on Apple platforms. On Android, the service is not available. Guard with platform checks or conditional DI resolution.
15. **Add `NSMicrophoneUsageDescription`** — required in `Info.plist` for `IMusicIdentifier` to work. The library requests permission automatically, but the plist key must be present or the app will crash.
16. **Playlist CRUD is on `IMediaLibrary`** — use `CreatePlaylistAsync`, `RemovePlaylistAsync`, `AddTrackToPlaylistAsync`, `RemoveTrackFromPlaylistAsync`. On both platforms these manage locally-stored custom playlists.
...
```csharp
// Identify a song playing nearby (Apple platforms only)
